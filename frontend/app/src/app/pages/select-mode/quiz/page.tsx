  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-blue-100 to-indigo-100 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl">
        <h2 className='text-3xl font-bold text-center mb-8 text-indigo-800'>ジャンルを選択する</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {genreDataList.map((genre) => (
            <Link href={`/pages/select-mode/quiz/${genre.id}`} key={genre.id} className="block">
              <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105">
                <Genre genre={genre} />
              </div>
            </Link>
          ))}
